...
# PATH_TO_LOCAL_DS = "/home/aguiar/Documents/SE_2024_Task_2/SemEval-2024-Task-2/training_data/dump"
...
def main():
    ...
    if training_args.do_train:
        ...
        """
        load_from_disk(  # TODO check, I used the local stored version here load_dataset
            PATH_TO_LOCAL_DS,
            split="train",
    ...
    # For initializing a new model for each optimization try 
    ...
    """
    def concat_input_seq(premise):
        concat_premise = ""
        for i in premise:
            concat_premise = concat_premise + str(i) + '\n'
        return concat_premise
    """
    ...
    def concat_input_seq(example):
        ...
        # print("THE BUILT PREMISE", concat_premise)
    ...
    def optuna_hp_space(trial):
        return {
            "learning_rate": trial.suggest_float("learning_rate", 1e-6, 1e-4, log=True),  # TODO change the values
            "per_device_train_batch_size": trial.suggest_categorical("per_device_train_batch_size", [64]),  #4, 8, 16, 
    ...
    trainer = Trainer(
        ...
        model_init=model_init  # for optimization 
    )
    ...
    if training_args.do_train:
        ...
        best_trial = trainer.hyperparameter_search(
            direction=["minimize", "maximize"],  # "minimize" --> when loss, but when acc we use maximize 
            backend="optuna",
        ...
        trainer.save_state()
        # optuna plots 
        fig1 = plot_optimization_history(best_trial)
        fig2 = plot_param_importances(best_trial)

        fig1.savefig("opti_history.png")
        fig2.savefig("param_importance.png")  
